...
def run_model(model, running_mode='train', train_set=None, valid_set=None, test_set=None,
              ...
              criterion=nn.CrossEntropyLoss(), device=torch.device('cuda')):
    """
...
def _train(model, train_loader, valid_loader, epoch_num, learning_rate=10e-5, stop_thr=1e-4,
    ...
    print("----run_model----learning_rate-----",learning_rate)
    ...
    for epoch in range(epoch_num):
      #
      print("--run_model---epoch-num-",epoch)

      # Train model
      model.train()
      print("--run_model--START---model.train()--")
      losses, nums = zip(*[loss_batch(model, loss_func, x, y, optimizer) for x, y in train_loader])
      train_loss = np.sum(np.multiply(losses, nums)) / np.sum(nums)
      print("--run_model---train_loss----",train_loss)
